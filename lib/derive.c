...
static const char *wordlist[WORDLIST_SIZE] = {
    "abandon",
    "ability",
    "able",
    "about",
...
bool pwsec_derivebytes(const char *mnemonic, OUT uint8_t key[6])
{
    ...
    for (i = 0; i < 4; i++) // for each mnemonic word
    {
        ...
        while (*mnemonic == ' ') // skip spaces
            mnemonic++;

        x[i][j] = 0; // null-terminate
    }

    for (k = 0; k < WORDLIST_SIZE; k++) // for each word in wordlist
    {
        if (!a && (a = (strcmp(x[0], wordlist[k]) == 0))) // search only if not found yet
            u.parts.a = k;
        if (!b && (b = (strcmp(x[1], wordlist[k]) == 0))) // search only if not found yet
            u.parts.b = k;
